...
makeSuite('Mainnet Check list', (testEnv: TestEnv) => {
  ...
  it('Deposit WETH', async () => {
    ...
    const oTokensBalance = await aWETH.balanceOf(user.address);

    expect(oTokensBalance).to.be.gt(zero);
    expect(oTokensBalance).to.be.gte(depositSize);
  });
  ...
  it('Withdraw WETH - Partial', async () => {
    ...
    expect(oTokensBalance).to.be.gt(zero, 'User should have oTokens.');

    // Partially withdraw native ETH
    const partialWithdraw = await convertToCurrencyDecimals(aWETH.address, '2');

    // Approve the oTokens to Gateway so Gateway can withdraw and convert to Ether
    ...
    const afterPartialOTokensBalance = await aWETH.balanceOf(user.address);
    ...
    expect(afterPartialOTokensBalance).to.be.equal(
      oTokensBalance.sub(partialWithdraw),
  ...
  it('Withdraw WETH - Full', async () => {
    ...
    const afterFullOTokensBalance = await aWETH.balanceOf(user.address);
    const gasCosts = approveGas.add(withdrawGas).mul(approveTx.gasPrice);

    expect(afterFullEtherBalance).to.be.eq(
      priorEthersBalance.add(oTokensBalance).sub(gasCosts),
      'User ETHER balance should contain the full withdraw'
    );
    expect(afterFullOTokensBalance).to.be.eq(0, 'User aWETH balance should be zero');
  });
  ...
  it('Borrow stable WETH and Full Repay with ETH', async () => {
    ...
    const oTokensBalance = await aDai.balanceOf(user.address);

    expect(oTokensBalance).to.be.gt(zero);
    expect(oTokensBalance).to.be.gte(daiSize);
